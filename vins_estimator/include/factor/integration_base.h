...
class IntegrationBase
{
  public:
          ...
          delta_eta{Eigen::Vector3d::Zero()}
    {
        ...
        // compute IMU and encoder's Jacobian and Variance together.
        noise = Eigen::Matrix<double, 18, 18>::Zero();
        noise.block<3, 3>(0, 0) =  (ACC_N * ACC_N) * Eigen::Matrix3d::Identity();
        noise.block<3, 3>(3, 3) =  (GYR_N * GYR_N) * Eigen::Matrix3d::Identity();
        noise.block<3, 3>(6, 6) =  (ACC_N * ACC_N) * Eigen::Matrix3d::Identity();
                            ...
                            bool update_jacobian)
    {
        ...
        result_delta_v = delta_v + un_acc * _dt;
        // encoder 
        Vector3d un_enc_0 = delta_q * RIO * _enc_v_0; 
        Vector3d un_enc_1 = result_delta_q * RIO * _enc_v_1; 
        result_delta_eta = delta_eta + 0.5 * (un_enc_0 + un_enc_1) * _dt;
    ...
    std::vector<Eigen::Vector3d> enc_v_buf;
